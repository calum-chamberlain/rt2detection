...
void TRIGGER_TIME_S(char timestamp_trg[],int ws,int ss,int kanal,float dt)
{
  ...
  sprintf(timestamp,"%2d/%2d/%2d %2d:%2d:%6.3f\0",iday,imon,iyrn,ihrn,imin,sek);
  ...
  sprintf(TRIGGER_TIMES_S[kanal],"%s\0",timestamp);
...
void TRIGGER_TIME_P(char timestamp_trg[],int ws,int pindex,int kanal,float dt)
{
  ...
  sprintf(TRIGGER_TIMES_P[kanal],"%s\0",timestamp);
...
void Scale_to_100_Raw(int no_of_smp,int cmpkl,float MxAmp)                 // Scale channel to max 1
{
  ...
  for(k=0;k<no_of_smp;k++)
  {
    vraw[cmpkl][k]=(vraw[cmpkl][k]/MxAmp)*100.0;
  }
...
void Scale_to_100_Flt(int no_of_smp,int cmpkl,float MxAmp)                 // Scale channel to max 1
{
  ...
  for(k=0;k<no_of_smp;k++)
  {
    vflt[cmpkl][k]=(vflt[cmpkl][k]/MxAmp)*100.0;
  }
...
#endif
//------------------------------------------------------------------------
// read pic.def if exist, otherwise use defaults
//------------------------------------------------------------------------
Read_Parameters();
...
//--------------------------------------------------------------------
  sprintf(sfilepath,"%s\0",sfilename);
  if(prt >= 1)  
    printf("rtpick.........................: %s\n",sfilepath);
  sprintf(cmd03,"cp %s s_orginal.sav\0",sfilepath);                  // save original s-file to current directory
  if(prt >= 1)
...
//-------------------------------------------------------------------
  ...
  for(i=0;i<50;i++)
  {
    ...
    if(record[79]=='1')
    {
      ...
      sprintf(MINTRGTID,"%s/%s/%s0%s:%s:%s\0",da_trg,mo_trg,yr_trg,hr_trg,mi_trg,se_trg);
      printf("MINTRGTID: %s\n",MINTRGTID);
    }
    if(record[79]=='6')
      break;
...
//--------------------------------------------------------------------------------------
  ...
  sprintf(cmd02,"%s/WAV/%s/%s/%s/%s\0",topdir,dbname,yr_trg,mo_trg,fnam);
  printf("CMD02: %s\n",cmd02);
      /* Add the file name to the intput file list */
...
//---------------------------------------------------------------------------------------
  ...
  for(i=0;i<nchannels;i++)
  {
    ...
    sprintf(trig_stations[i],"%s\0",comp);

    for(k=0;k<10;k++)
      comp[k]='\0';
    for(k=0;k<3;k++)
      comp[k]= STATION_NAMES[i][k+12];
    sprintf(trig_components[i],"%s\0",comp);
...
for(k=0;k<nchannels;k++)
{
  ...
  switch(readings)
  {
    ...
    sprintf(TRIGGER_TIMES_P[k],"                         \0");
    sprintf(TRIGGER_TIMES_S[k],"                         \0");
    if(prt >= 2)
    ...
    if(trig_components[k][2] == 'Z')
    {
      ...
      temptid[21]='\0';
      sprintf(TRIGGER_TIMES_P[k],"%s\0",temptid);
      sprintf(TRIGGER_TIMES_S[k],"                         \0");
      if(prt >= 2)
    ...
    if(trig_components[k][2] == 'Z')
    {
      ...
      temptid1[21]='\0';
      sprintf(TRIGGER_TIMES_P[k],"%s\0",temptid1);
      sprintf(TRIGGER_TIMES_S[k],"                         \0");
      if(prt >= 2)
      ...
      temptid2[21]='\0';
      sprintf(TRIGGER_TIMES_P[k],"                         \0");
      sprintf(TRIGGER_TIMES_S[k],"%s\0",temptid2);
      if(prt >= 2)
    ...
    if(trig_components[k][2] == 'Z')
    {
      ...
      temptid3[21]='\0';
      sprintf(TRIGGER_TIMES_P[k],"                         \0");
      sprintf(TRIGGER_TIMES_S[k],"%s\0",temptid3);
      if(prt >= 2)
...
//------------------------------------------
  for(cmpix = 0; cmpix < nchannels; cmpix++)
  {
    sprintf(timestring,"%s\0",TIME_START[cmpix]);
...
//---------------------------------------
  for(cmpix = 0; cmpix < nchannels; cmpix++)
  {
    ...
    dum[2]='\0';
    sprintf(COMP_NAMES[cmpix],"%s\0",dum);
  }
  sprintf(dum,"hyp.out\0"); 
...
//---------------------------------------
  for(cmpix = 0; cmpix < nchannels; cmpix++)
  {
    ...
    sprintf(NAME_ONLY[cmpix],"%s\0",fil);
  } 
...
//-----------------------------------------------------------------
  ...
  {
    sprintf(timestring,"%s\0",HYPFILE[nl]);
    ...
    sscanf(timestring,"%5c%s",dum,comp);
...
for(cmpix = 0; cmpix < nchannels; cmpix++)
{
  ...
  if(ret != 0)                                             // New station ?
  {
    ...
    sprintf(pngfile,"%s/loc/%s.png\0",topdir_rt,fil);   // YES, file name format: BER.png or PB10.png
    sprintf(last_station,"%s\0",fil);
    im  = gdImageCreate(795, 345);                        // Allocate image: tot_wd pixels width by tot_hg pixels high
    ...
    ix_old=idx;
    iy_old=avstand;
    gdImageLine(im, 0, 0, 794, 0, sg4);                     // draw frame  
    gdImageLine(im, 794, 0, 794, 344, sg4);                 // draw frame
    gdImageLine(im, 794, 344, 0, 344, sg4);                 // draw frame
    gdImageLine(im, 0, 345, 0, 0, sg4);                     // draw frame
//------------------------------------------------- 
...
//---------------------------------------------------   
    ...
    sprintf(timestring,"%s\0",TIME_START[kan]);
    ...
    sprintf(sec10,"%2d\0",ret);
    incr = (float)794/((float)rate * ((float)nemin/rate));
    ...
    for(i=0;i<nusmp[cmpix];i++)
    {
      ...
      if(i==mark10)
      {
        gdImageLine(im, ix_new, 60, ix_new, 335, tl);      // draw 10 seconds marks		    
        if(sec10[0]==' ')
...
im  = gdImageCreate(795, 2500);                 // Allocate the image: tot_wd pixels width by tot_hg pixels high
bg  = gdImageColorAllocate(im, 255, 255, 255); // WHITE first color definition defines the background color
tl  = gdImageColorAllocate(im, 200, 200, 200); // GRAY  color for timelines
tk  = gdImageColorAllocate(im,   0,   0,   0); // BLACK color for text
tt  = gdImageColorAllocate(im,   0,   0, 255); // BLUE color for title
...
yincr = 50;
avstand=yincr;
sprintf(pngfile,"%s/loc/ALL.png\0",topdir_rt);     // file to write png
...
for(cmpix = 0; cmpix < nchannels; cmpix++)
{
  ...
  gdImageLine(im, 0, 0, 794, 0, dg);                       // plot frame
  gdImageLine(im, 794, 0, 794, 2450, dg);                   // plot frame
  gdImageLine(im, 794, 2450, 0, 2450, dg);                   // plot frame
  gdImageLine(im, 0, 2450, 0, 0, dg);                       // plot frame
  ...
  idx = ix_old;
  ix=nusmp[cmpix];
  sprintf(dum,"%s\0",STATION_NAMES[cmpix]);
  strncpy(nam,dum,12);
...
//      printf("%s",linjer);
    ...
    while ( fgets ( linjer, sizeof linjer, sf ) != NULL ) /* read a line from file */
    {
     if(prt >= 1)   
        printf("%s",linjer);
      sprintf(HYPFILE[ant],"%s\0",linjer);
      ant++;
    }
    fclose(sf);
}
...
Read_Parameters()
{
  ...
  if(par == NULL)
  {
    printf("pic.def does not exist in current directory.\n");
    sprintf(dummy,"%s/DAT/pic.def\0",topdir);
    par = fopen(dummy,"r");
    if(par == NULL)
    {
      printf("pic.def does not exist in %s. Use defaults.\n",topdir);
      br=1;
    }
  }
  if(br == 0)
...
//	prt = (int)val;	
    ...
    rewind(par);    
    sprintf(keyword,"MAX RESIDUAL\0");
    while ( fgets ( linjer, sizeof linjer, par ) != NULL ) /* read a line from file */
    {
...
//  sf=fopen(sfilepath,"r");
      ...
      for(l=0;l<90;l++)
      {
        ...
        if(linjer[l] == EOF)
        {
          ...
          if(linjer[l] == 0xa )
          {
            ...
            for(n=0;n<6;n++)
              navn[n]=linjer[n];
            sprintf(toplot1[i], "%s\0",navn);
            sprintf(HYPFILE[i],"%s\0",linjer);
  ...
  printf("readings left: %2d Avg.res: %8.2f  ",ant,residual);
  *resid = residual;
  *nst   = ant;
  for(l=0;l<200;l++)
    index[l]=0;
  ...
  if(sf == NULL)
  {
    printf("CHECK_S: Can't open hyp.out\n");
    exit(0);
  }else{
//    printf("Found the: hyp.out\n");
  ...
  if(out == NULL)
  {
    printf("CHECK_S: Can't open hyp_new.out\n");
    exit(0);
  }else{
    ...
    *ne=ant;
    ...
    if( (residual/ant) > res_lim)
    {
      ...
      printf("STOP iterations. Residual below: %8.2f\n",res_lim);
      goon=0;  // Stop
...
//            if(mail == 1)
//            {
//	      sprintf(comm,"echo %c%s%c | sendmail %s\0",0x22,linjer,0x22,mailaddress1);
//	      ret=system(comm);
//            }
//	  }
...
Update_map_file(char snam[],int ant,float resid,int nst,int klon)
{
  FILE *sf;
  FILE *loc;
  FILE *out;
...
//    printf("Found the: hyp.out\n");
  ...
  sprintf(mapout,"%s/map/map.out\0",topdir_rt); 
  out=fopen(mapout,"r+");
  if(out == NULL)
  {
  ...
  sprintf(mapname,"%s/map/ALL_EPI%d.txt\0",topdir_rt,klon);
  gog=fopen(mapname,"a+");
  if(gog == NULL)
  {
    printf("rtpick:Update_map_file: Can't open %s\n",mapname);
...
// write cdate, coordinates,depth and magnitude for gmt map 
  ...
  sprintf(gmtmap,"%s/map/GMT_EPI%d.txt\0",topdir_rt,klon);
  gmt=fopen(gmtmap,"a+");
  if(gmt == NULL)
  {
    printf("rtpick:Update_map_file: Can't open %s\n",gmtmap);
    ...
    sprintf(mapname,"%s/loc/%s.html\0",topdir_rt,snam);
    loc=fopen(mapname,"w");
...
printf("mail1: %d\n",mail1);
    if(mail1 == 1)
    {
      for(l=0;l<500;l++)
        mail_message[l]='\0';
//sprintf(mail_message,"echo %chttp://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=7&size=900x1000&maptype=roadmap&markers=color:red_7Ccolor:red_7Clabel:Q_7C%f,%f&sensor=false%c | mutt -s %cTRIGGER%c -- %s &\0",0x22,lat,lon,lat,lon,0x22,0x22,0x22,mailaddress1);
//          sprintf(mail_message,"echo %cQUAKE%c | mutt -a %c%s/rtquake/map/hyp.out%c -s %cTRIGGER%c -- %s &\0",0x22,0x22,0x22,topdir_rt,0x22,0x22,0x22,mailaddress1);

sprintf(mail_message,"echo %chttp://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=7&size=900x1000&maptype=hybrid&markers=color:red_7Ccolor:red_7Clabel:Q_7C%f,%f&sensor=false%c | mutt  -a %c%s/loc/ALL.png%c -s %cTRIGGER%c -- %s &\0",0x22,lat,lon,lat,lon,0x22,0x22,topdir_rt,0x22,0x22,0x22,mailaddress1);
printf("%s\n",mail_message);
      for(l=0;l<500;l++)
      {
        if(mail_message[l]=='_')
...
sprintf(mail_message,"echo %chttp://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=7&size=900x1000&maptype=hybrid&markers=color:red_7Ccolor:red_7Clabel:Q_7C%f,%f&sensor=false%c | mutt  -a %c%s/loc/ALL.png%c -s %cTRIGGER%c -- %s &\0",0x22,lat,lon,lat,lon,0x22,0x22,topdir_rt,0x22,0x22,0x22,mailaddress2);
printf("%s\n",mail_message);
      for(l=0;l<500;l++)
      {
        if(mail_message[l]=='_')
...
//      printf("Mail sent\n");
  ...
  sprintf(locname,"%s/map/LAST_LOC.txt\0",topdir_rt);
  llc=fopen(locname,"w");
  if(llc == NULL)
  {
    printf("rtpick:Update_map_file: Can't open %s\n",locname);
...
//---------------------------------------------
  ...
  if(locate == 1 && keep == 0)
  {
    printf("Create_Sfile: Locate + new s-file.\n");

    sprintf(com13,"automag %s\0",sfilepath);
    sprintf(com14,"hyp automag.out >> hyptemp.txt\0");
    sprintf(com15,"cp automag.out %s\0",sfilepath);

    sprintf(comm0,"rm hyptemp.txt\0");
    sprintf(comm1,"hyp %s >> hyptemp.txt\0",sfilepath);
    sprintf(comm2,"cp hyp.out %s\0", sfilepath);
    sprintf(comm3,"cp hyp_new.out %s\0", sfilepath);
    sprintf(comm4,"cp %s hyp_save.out\0",sfilepath);
    sprintf(comm5,"cp hyp_save.out %s\0",sfilepath);
    sprintf(comm6,"cp hyp.out hyp.tmp\0");
    sprintf(comm7,"cp hyp.tmp %s\0",sfilepath);
    sprintf(comm8,"cp hyp.out hyp_all.out\0");
    sprintf(comm9,"cp hyp_save.out %s\0",sfilepath);
    sprintf(com10,"cp %s s_org.out\0",sfilepath);
#ifdef RTQUAKE
    sprintf(com11,"cp hyp.out %s/map\0",topdir_rt);
#endif    
...
///      ret=system(comm7);
	...
	{
//--------------AUTOMAG------------------------------------------------- 
          printf("AM rtpick:comm0....................: %s\n",comm0);
          ret=system(comm0);                                     // remove old hyptemp.txt
          printf("AM rtpick:com10....................: %s\n",com10);
          ret=system(com10);                                     // copy original s-file to s_org.out
//    printf("rtpick:com15....................: %s\n",com15); // set DEF_BASE
//    ret=system(com15);
//    printf("RET: %d\n",ret);
          printf("AM rtpick:com13....................: %s\n",com13);    
          ret=system(com13);                                     // run automag, creates automag.outfile
          printf("AM rtpick:com15....................: %s\n",com15);
          ret= system(com15);                                    // copy automag.out new s-file

          printf("AM rtpick:com14....................: %s\n",com14);
          ret=system(com14);                                     // run hyp, input automag.out, this produces "hyp.out"       
          printf("AM rtpick:comm2....................: %s\n",comm2);
          ret=system(comm2);                                     // copy hyp.out new s-file
          printf("AM rtpick:comm8....................: %s\n",comm8);
          ret=system(comm8);                                     // copy hyp.out to hyp_all.out
//--------------END AUTOMAG---------------------------------------------    
	}
//        if(mail == 1)
//        {
//          sprintf(dummy,"echo %cQUAKE%c | mutt -a %c%s/rtquake/map/hyp.out%c -s %cTRIGGER%c -- %s &\0",0x22,0x22,0x22,topdir_rt,0x22,0x22,0x22,mailaddress1);
//	  printf("%s\n",dummy);
//          ret=system(dummy);
//        }	
      }
...
//      fprintf(sf,"%s\n",record);
        ...
        fclose(sf);
        sprintf(com12,"cp -f comb_sfile %s\0",sfilepath);
	printf("COM12: %s\n",com12);
        ret=system(com12);        
...
//printf("%s %f\n",seconds,SEC);
  ...
  printf("S_REC:fullpath..................: %s\n",fullpath);

   

  if(keep == 1)
  {
    printf("S_REC: Read old readings from: s_original.sav\n");    
    so=fopen("s_orginal.sav","r");       // open original s-file
    if(so == NULL)
    {
      printf("S_REC: Can't open original s-file: s_original.sav.\n");
      exit(0);
    }
  }
  printf("S_REC: Create new s-file........: %s\n",fullpath);
  sf=fopen(fullpath,"w");
  if(sf == NULL)
  {
    printf("S_REC: Can't open S-file: %s\n",fullpath);
    for(i=0;i<52;i++)
  ...
  if(keep==1)
  {
    printf("S_REC: Copy s_original.sav on top in new s-file.\n");
//----------------------------------------------------------------------
...
//      fprintf(sf,"%s\n",record);
  ...
  if(keep == 0)
  {
    printf("S_REC: Write new s-file header to s-file.\n");
    for(i=0;i<100;i++)
    buf[i]=' ';
...
//        fprintf(fp, "%s\n", pick_str);
//        printf("%s\n",pick_str);
        sprintf(PICKLINES[n],"%s\0",pick_str);
    }
if(prt >=0)
{
    for(n=0;n<num_picks_definative; n++)
...
//printf("%3d %s %6d %4d %s\n",cmp,station,nsamp,sr,timestamp);
  sprintf(STATION_NAMES[cmp],"%s\0",station);
  sprintf(TIME_START[cmp],"%s\0",timestamp);
  TIME_START[cmp][10] = ' ';
  for(i=0;i<MAXSMP;i++)
  {
    fseismo[i]=0;
  ...
  srates[cmp]=sr;
//  sprintf(TRIGGER_STATIONS[cmp],"%s\0",station);
...
// where the XX represent missing networkname
  ...
  {
    stdstation[0]  = current_station[0];     // network name
    stdstation[1]  = current_station[1];     // network name
    stdstation[2]  ='_';
...
if(prt == -10)
{
      ...
      sprintf (COMP_HDR[chan], "TIMESERIES %s , %lld samples, %g sps, %s , SLIST, %s\0",
	       srcname, (long long int)mst->numsamples, mst->samprate, timestr, samptype);
