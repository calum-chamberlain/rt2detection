...
Read_Parameters()
{
  ...
  {
    if(par > 0)
      printf("rtpick: pic.def does not exist in current directory.\n");
    sprintf(dummy,"%s/DAT/pic.def",topdir);
    par = fopen(dummy,"r");
    if(par == NULL)
    {
      if(par > 0)     
        printf("rtpick: pic.def does not exist in %s. Use defaults.\n",dummy);
      br=1;
    }
  }
  if(br == 0)
...
printf("mail1: %d\n",mail1);
    if(mail1 == 1)
    {
      for(l=0;l<500;l++)
        mail_message[l]='\0';
//sprintf(mail_message,"echo %chttp://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=7&size=900x1000&maptype=roadmap&markers=color:red_7Ccolor:red_7Clabel:Q_7C%f,%f&sensor=false%c | mutt -s %cTRIGGER%c -- %s &",0x22,lat,lon,lat,lon,0x22,0x22,0x22,mailaddress1);
...
// GE_ABCDE_10_HHZ_D  
  if(station[0] != '_')                      // check if station name does NOT contain network name
  {
    ...
    if(station[2] == '_' && station[8] == '_' && station[12] == '_')  // CX_PB099_BHE_D 
    {
      stdstation[0]  = current_station[0];     // networkname
      stdstation[1]  = current_station[1];     // networkname
      stdstation[2]  = '_';
